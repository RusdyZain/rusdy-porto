...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all cantures.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*cantures\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@cantures.com" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@cantures.com" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
