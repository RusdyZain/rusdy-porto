...
const Skills = () => {
  ...
  const categories = [
    { id: 'frontend', name: 'Frontend', color: 'from-blue-500/20 to-cyan-500/20' },
    { id: 'backend', name: 'Backend', color: 'from-green-500/20 to-emerald-500/20' },
    { id: 'mobile', name: 'Mobile', color: 'from-purple-500/20 to-pink-500/20' },
    { id: 'database', name: 'Database', color: 'from-orange-500/20 to-amber-500/20' },
    { id: 'devops', name: 'DevOps', color: 'from-red-500/20 to-rose-500/20' },
    { id: 'design', name: 'Design', color: 'from-indigo-500/20 to-violet-500/20' }
  ];
          ...
          {categories.map((category) => {
            ...
            return (
              <div key={category.id} className="space-y-6">
                {/* Category Header */}
                <h3 className="text-xl font-semibold text-white/80">{category.name}</h3>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {categorySkills.map((skill) => (
                    ...
                    >
                      {/* Glow Effect */}
                      <div className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${category.color} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}></div>
